        ...
        <table class="min-w-full overflow-hidden divide-y divide-gray-300 rounded-xs">
            <thead class="text-white bg-primary-700">
                <tr class="font-semibold tracking-wider text-left uppercase lg:text-subtitle">
                    ...
                    <th scope="col" class="w-5/12 px-6 py-3">Nama Lengkap</th>
                    <th scope="col" class="w-6/12 px-6 py-3">Jabatan</th>
                </tr>
            </thead>
            <tbody class="bg-white">
                @foreach ($staff as $index => $member)
                    <tr
                        class="{{ $loop->odd ? 'bg-white border-y border-mist-300' : '' }} bg-primary-50 divide-x divide-gray-200">
                        <td class="px-6 py-2 text-center text-wrap">{{ $member->id }}</td>
                        <td class="px-6 py-2 capitalize text-wrap">{{ $member->name }}</td>
